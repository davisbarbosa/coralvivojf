...
const SPSCorals = () => {
        ...
        readString(csvString, {
          ...
          complete: (results) => {
            setProducts(
              results.data.filter(
                (product) =>
                  product.type === "coral" && product.subtype === "sps"
              )
            );
            setLoading(false);
          },
  ...
  return (
    <>
      ...
      >
        <Container>
          <h1 className="text-center mb-4">Corais SPS</h1>

          {/* Controle de Ordenação */}
          <Row className="mb-3">
            <Col md={{ span: 2, offset: 8 }} className="d-flex flex-wrap justify-content-md-end">
              <Button
